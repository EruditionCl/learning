...
def detector(func, final=None):
    if final is None:
        final = {}
    if isinstance(func, Variable):
        final[f"{func}"] = final.get(f"{func}", 0) + 1
    elif isinstance(func, Constant):
        final["1"] = final.get("1", 0) + func.value
    elif isinstance(func, Symbol):
        final[f"{func}"] = final.get(f"{func}", 0) + 1
    elif isinstance(func, Add):
        final = detector(func.x)
        for key, value in detector(func.y).items():
            final[key] = final.get(key, 0) + value  
    elif isinstance(func, Multiply):
        if isinstance(func.x,Constant):
            result = detector(func.y, {})
            coeff = func.x.value
            for key in result:
       ...
       final[f"{func}"] = final.get(f"{func}", 0) + 1
    return dict(sorted(final.items(),reverse=True))

def translator(final):
    coeffs=[]
    for key in final:
        if key=="π":
            coeffs.append(final[key])
